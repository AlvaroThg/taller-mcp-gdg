// CHECKPOINT 5 (BONUS) -- + Tool "buscar_por_titulo"
// Solo si en el taller se llegó a agregar el segundo tool opcional.
...
const BuscarPorTituloSchema = {
  titulo: z
    .string()
    .min(2)
    .describe("Título o parte del título de la película a buscar. Ejemplo: 'Inception' o 'Matrix'"),
};

server.tool(
  "buscar_por_titulo",
  "Busca una película por su título o parte de él. Úsalo cuando el usuario mencione el nombre de una película específica.",
  BuscarPorTituloSchema,
  async (params) => {
    const apiKey = process.env.TMDB_API_KEY;
    if (!apiKey) {
      return { content: [{ type: "text" as const, text: "Error: TMDB_API_KEY no configurada." }] };
    ...
    url.searchParams.set("api_key", apiKey);
    url.searchParams.set("language", "es-ES");
    url.searchParams.set("query", params.titulo);
    ...
    if (peliculas.length === 0) {
      return {
        content: [{ type: "text" as const, text: `No encontré ninguna película con el título "${params.titulo}".` }],
      };
    }
    ...
    return {
      content: [{ type: "text" as const, text: `Resultados para "${params.titulo}":\n\n${texto}` }],
    };
  }
);
